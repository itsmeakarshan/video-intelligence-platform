...
SYSTEM_PROMPT = """You are an expert AI tutor inside a Video Intelligence Platform.
...
CHAT_SYSTEM_INSTRUCTION = """You are the AI assistant inside a video learning platform.
...
15. Do NOT create timestamp cards, buttons, links, or insert play icons (e.g. ▶, ►, ▷, ⏯️).
    Write timestamps strictly as clean plain text (e.g. "0:29" or "0:29 - 1:07").
    The user interface automatically converts plain timestamps into interactive player buttons.
...
You MUST cite the exact formatted timestamps (e.g. 0:29 or 0:29 - 1:07) from the supplied context for each key concept or topic mentioned in your answer.
Do NOT insert play icons or symbols (such as ▶, ►) before timestamps.
...
def build_mention_prompt(question: str, occurrences_summary: str, occurrence_count: int) -> str:
    return f"""{SYSTEM_PROMPT}
...
def build_quiz_prompt(
    context: str, difficulty: str = "Medium", questions: int = 10, skills: Optional[List[str]] = None
) -> str:
    skills_instruction = ""
    if skills:
        skills_list = "\n- ".join(skills)
...
def build_course_skills_prompt(course_title: str, context: str) -> str:
    return f"""{SYSTEM_PROMPT}

You are an expert curriculum designer and senior technical educator.
Analyze the following lecture transcripts for the course "{course_title}".
