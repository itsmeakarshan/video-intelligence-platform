"""
Learning Analytics & A/B Experiment Service for Video Intelligence Platform.

Tracks:
1. Learning Gain: delta = post_recommendation_score - pre_recommendation_score
2. Pilot A/B Experiment Framework (Control vs Personalised Recommendation Treatment)
"""
...
def compute_learning_gain(user_id: int, db: Session) -> dict:
    """
    Computes score improvement associated with quiz attempts after recommendations.
    """
    attempts = (
    ...
    mean_gain = round(float(np.mean(gains)), 1)
    pre_avg = round(float(np.mean(pre_scores)), 1)
    post_avg = round(float(np.mean(post_scores)), 1)
...
def get_ab_experiment_summary(db: Session) -> dict:
    """
    Returns A/B Experiment pilot analytics comparing Control vs Treatment groups.
    """
    attempts_all = db.query(QuizAttempt).all()
    total_sample = len(attempts_all)
